...
bool isSafeContainerPath(const std::string& path)
{
    bool endsWithParent = path.size() >= 3 && path.compare(path.size() - 3, 3, "/..") == 0;
    return !path.empty()
        && path.front() == '/'
        && path != "/"
...
bool setupBindMounts(const RuntimeConfig& config)
{
    for (const auto& [destination, source] : config.bindMounts) {
        if (!isSafeContainerPath(destination)) {
            std::cerr << "Invalid bind mount destination " << destination << "\n";
            return false;
        }

        std::string target = config.rootfs + destination;
        if (!mkdirRecursive(target, 0755)) {
            std::cerr << "Failed to create bind mount target " << target << ": " << strerror(errno) << "\n";
            return false;
        }

        if (mount(source.c_str(), target.c_str(), nullptr, MS_BIND | MS_REC, nullptr) == -1) {
            std::cerr << "bind mount failed for " << source << " -> " << destination << ": " << strerror(errno)
                      << "\n";
            return false;
        }
    }
